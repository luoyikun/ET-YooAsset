...
namespace ET
{
    ...
    public class DataModifierComponent: Entity, IAwake, IDestroy, IUpdate, IFixedUpdate
    {
        #region 私有成员
        ...
        public void AddDataModifier(string modifierName, ADataModifier dataModifier, NumericType numericType = NumericType.Min)
        {
            ...
            if (numericType == NumericType.Min)
                return;
            this.GetParent<Unit>().GetComponent<NumericComponent>()[numericType] =
                    this.BaptismData(modifierName, this.GetParent<Unit>().GetComponent<NumericComponent>().GetOriNum()[(int)numericType]);
        }
